...
mymatriz *mmultiplicarvThread (mymatriz *mat_a, mymatriz *mat_b, int tipo) {
	...
	if (divid < 8){ // aqui vamos tentar ver quantas threads
		n_threads = divid;
	}

	divid = divid/n_threads;
	resto = divid % n_threads;
	start = 0;
	last = 0;


	threads = (pthread_t *) malloc (n_threads * sizeof(pthread_t));
	args = (param_t1 *) malloc (n_threads * sizeof(param_t1));

	for( int i = 0; i < n_threads; i++){
		//printf("\nCriando Thread %i\n", i);
		if( resto > 0){
			last = divid + i*divid + 1;
		}else{
			last = divid + i*divid;
		}
		//printf("Thread %i ira do bloco %i ao %i\n", i, start, last);
		args[i].bloco_a = criaBloco(mat_a->matriz, 0	, nLinha, 	start,	last);
		args[i].bloco_b = criaBloco(mat_b->matriz, start, last, 0, nColuna);
		args[i].nLinha = nLinha;
		args[i].nColuna = nColuna;
		args[i].mat_c = NULL;
		args[i].id = i;
		pthread_create( &threads[i], NULL, mmultiplicarBloco_worker, (void *) (args+i));
		//printf("\nSoltando Thread %i\n", i);
		start = last;
		resto--;
	}
	...
	for (int i = 1; i < n_threads; ++i) {
		mat_aux = mat_c;
		mat_c = msomar( mat_c, args[i].mat_c, 0);
		mliberar(mat_aux);
		free(mat_aux);
...
void *mmultiplicarBloco_worker( void *args){
	...
	//printf("Inicio %i\n", p->id);
	p->mat_c = mmultiplicarBlocos( p->bloco_a, p->bloco_b, p->nLinha, p->nColuna, 0);
	//mimprimir(p->mat_c);
	//printf("Fim %i\n", p->id);
	return NULL;
}

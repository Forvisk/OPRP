...
*/
	...
	for (int ii=0; ii < N_EXEC; ii++) {
		printf("\n ##### multiplicar_t%d de Matrizes Em OpenMP#####\n", ii);
		start_time = wtime();
		mat_multT[ii] = mmultiplicarvOpenMP(&mat_a, &mat_b, ii);
		end_time = wtime();
...
*/
	...
	printf("\n ##### Comparação dos resultados da Multiplicação de matrizes por OpenMP#####\n");
	for (int i=1; i<N_EXEC; i++) {
		printf("[mult_t0 vs mult_t%d]\t", i);
